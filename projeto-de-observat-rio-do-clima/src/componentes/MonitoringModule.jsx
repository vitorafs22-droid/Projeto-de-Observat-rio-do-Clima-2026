...
export default function MonitoringModule() {
  ...
  const [status, setStatus] = useState(null); // 'normal' | 'warning'

  const handleAssessment = (e) => {
    e.preventDefault();

    // Regra simples para detecção precoce de descompensação:
    // Pouco sono (< 5h), medicação interrompida ou agitação/alucinações altas (>= 4)
    if ((Number(sleepHours) < 5 && sleepHours !== '') || medsTaken === 'nao' || Number(agitationLevel) >= 4) {
      setStatus('warning');
    } else {
      setStatus('normal');
    }
      ...
      <div style={{ backgroundColor: '#fff', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
        ...
        {status === 'warning' && (
          <div style={{ backgroundColor: '#fff3e0', borderLeft: '5px solid #f57c00', padding: '1rem', marginTop: '1.5rem', borderRadius: '4px' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.9rem', color: '#333' }}>
              Os parâmetros inseridos (sono reduzido, falha na medicação ou agitação) são fatores de risco para uma descompensação psicopatológica. Recomendamos alertar o cuidador ou contatar o CAPS/Médico de referência.
            </p>
          </div>
        )}
        ...
        {status === 'normal' && (
          <div style={{ backgroundColor: '#e8f5e9', borderLeft: '5px solid #2e7d32', padding: '1rem', marginTop: '1.5rem', borderRadius: '4px' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.9rem', color: '#333' }}>
              Parâmetros dentro da faixa normal. Continue mantendo a rotina de sono e o acompanhamento do tratamento.
            </p>
          </div>
        )}
